...
char questions[][25] = {"What is 5+1?", "What is 2+1?", "What is 1+3?"};
char answers[][25] = {"6", "3", "4"};
int no_of_ques = 3;

int maxtime_to_ans = 50;
			...
			while(1)
			{
				...
				if(accepterror == 0)
				{
					...
					else if(pid == 0)
					{
						...
						char wrong[50] = "Better Luck Next Time :( ";
						char late[50] = "Too Late human!! ;)";
						...
						{
							send(s_server_child,finish,sizeof(finish),0);
							...
							if (strcmp(ans,"-666")==0) // OS wink! wink! oink!!!
							{
								client.late++;
								printf("\nAttempt by %s : %s---LATE---%d sec to answer\n",client.name,ans,time_to_ans);
							...
							{
								client.correct++;
								printf("\nAttempt by %s : %s---CORRECT---%d sec to answer\n",client.name,ans,time_to_ans);
