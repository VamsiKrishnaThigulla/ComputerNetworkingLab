...
char minname[20]=" "; //holds the name of the winner
				...
				if(accepterror == 0)
				{
					...
					else if(pid == 0)
					{
						...
						for(int i=0;i<no_of_ques;i++)
						{
							...
							time(&end); //end timer
							time_t diff=end-start; //time taken
							
							client.avg_time += diff; //Total time taken by one user

							if(strcmp(ans,"-666")==0)
							{
								client.late++;
								printf("\nAnswer entered by %s: %s -- Late Answer \n", client.name, ans);
								send(s_server_child, late, sizeof(late), 0);
							}
							else if(strcmp(answers[i], ans) == 0)
							{
								client.correct++;
								printf("\nAnswer entered by %s: %s -- Correct \n", client.name, ans);
